...
function tree(targetDOMelement) { 
	...
	//Lays out and renders (format 2) hierarchy in "nest" ("key-values" format).
		...
		hierarchyGraph = d3
			...
			.sum(d=>d.size) //accessor for size data (e.g. circle size in a d3.pack)
			                //However, it's usually not required for tree (this adds the sum of all descendants' sizes and stores in node.value) 
